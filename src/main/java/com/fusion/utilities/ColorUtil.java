...
/**
 * Function to get a color name from rgb/hex value/awt color
 * 
...
public class ColorUtil extends SuperHelper {
	...
	/**
	 * Function to Get the closest color name from above list
	 * 
	 * 
	 * @author AF33811 Lachireddy Palla
	 * 
	...
	/**
	 * Function to Convert hexColor to rgb, then call getColorNameFromRgb(r, g, b)
	 * 
	 * 
	 * @author AF33811 Lachireddy Palla
	 * 
	...
	/**
	 * Function SubClass of ColorUtils. In order to lookup color name
	 * 
	 * 
	 * @author AF33811 Lachireddy Palla
	 * 
	...
	/**
	 * Function to get the color name of the text and verify the Expected color
	 * 
	 * @author AF33811 Lachireddy Palla
	 * 
	 * @since 14-August-2017
	 * @param testObject
	 *            Desired text field to verify the text color with the expected color
	 * @param strExpectedValue
	 *            Expected Value
	 *
	 */

	public static boolean verifyTextColor(WebElement testObject, String strExpectedValue) {

		try {

			String strcolor = testObject.getCssValue("color");

			strcolor = strcolor.replace("(", "").replace(")", "").replace("rgba", "");
			String colorArray[] = strcolor.split(",");
			...
			String strConfirmationTextColor = util.getColorNameFromRgb(r, g, b).trim();

			if (strConfirmationTextColor.equalsIgnoreCase(strExpectedValue))

			{
				log(PASS, "Verify Text Color", "Pass: Expected Color " + strExpectedValue + ", ActualColor " + strConfirmationTextColor + ". Text Color is matched", true);
				return true;
			} else {
				// Fail if the compared color is not matched with Expected color
				log(FAIL, "Verify Text Color", "Fail: Expected Color " + strExpectedValue + ", Actual Color " + strConfirmationTextColor + ". Text Color is not matched", true);
				setLastTestPassed(false);
				return false;
			}
		} catch (Exception excException) {
			log(FAIL, "Default function verifyTextColor", "Exception while executing verifyTextColor function", false);
			throw excException;
		}
	}

	/**
	 * Function to get the color name of the Button and verify the Expected color
	 * 
	 * @author AF33811 Lachireddy Palla
	 * 
	...
	public static boolean seVerifyButtonColor(WebElement testobject, String strExpectedValue) {

		try {
			String strclass = testobject.getAttribute("class").split(" ")[2].trim();
			String strButtonColor;
			...
			case "btn-primary":
				...
				break;
			case "btn-success":
				strButtonColor = "Green";
				break;
			case "btn-info":
				strButtonColor = "Light Green";
				break;
			case "btn-warning":
				strButtonColor = "Yellow";
				break;
			case "btn-danger":
				strButtonColor = "Red";
				break;
			default:
				strButtonColor = "Unknown Color";
			}
			if (strButtonColor.equalsIgnoreCase(strExpectedValue)) {
				log(PASS, "Verify Text Color", "Pass: Expected Color [" + strExpectedValue + "],  Actual Color [" + strButtonColor + "]. Text Color is matched as expected", true);
				return true;
			} else {
				// Fail if the compared color is not matched with Expected color
				log(FAIL, "Verify Text Color", "Fail: Expected Color [" + strExpectedValue + "],  Actual Color [" + strButtonColor + "]. Text Color is not matched", true);
				return false;
			}
		} catch (Exception excException) {
			log(FAIL, "Default function verifyTextColor", "Exception while executing verifyTextColor function", false);
	...
	/**
	 * Function to get the color name of the Button
	 * 
	 * @author AF33811 Lachireddy Palla
	 * 
	 * @since 06-August-2018
	...
	public static String seGetButtonColor(WebElement testobject) {
